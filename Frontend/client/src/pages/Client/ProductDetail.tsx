...
const ProductDetail = () => {
  ...
  useProductPolling(id, product, setProduct, setRelatedProducts, selectedVariant, setSelectedVariant);
  ...
  useEffect(() => {
    if (consultHistory.length > 0) {
      localStorage.setItem(`consultHistory_${id}`, JSON.stringify(consultHistory));
    }
  }, [consultHistory, id]);
  ...
  const handleConsult = async () => {
    ...
    try {
      ...
      setConsultHistory((prev) => {
        const newHistory = [...prev, { question: consultQuery, answer: data.explanation }].slice(-10); // Giới hạn 10 câu hỏi
        return newHistory;
      });
      setConsultQuery("");
    } catch (error: any) {
      toast.error(error.response?.data?.message || "Lỗi khi tạo nội dung tư vấn");
    } finally {
                  ...
                  <>
                    ...
                    <div className="d-flex align-items-center mb-2">
                      ...
                      <span className="ml-2 text-muted">
                        (10 Đánh giá) | <a className="text-primary" href="#">Đánh giá</a>
                      </span>
                    </div>
                    ...
                    <div className="add-to-cart">
                      ...
                      <button
                        ...
                        onClick={() => addToCart(product?._id, selectedVariant, quantity)}
              ...
              <div id="product-tab">
                ...
                <div className="tab-content">
                  <div id="tab01" className={`tab-pane ${activeTab === "tab01" ? "active" : "fade"}`}>
                    <div className="row">
                      <div className="col-md-12">
                        <p dangerouslySetInnerHTML={{ __html: product?.short_description || "Không có mô tả" }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div id="tab02" className={`tab-pane ${activeTab === "tab02" ? "active" : "fade"}`}>
                    <div className="row">
                      <div className="col-md-12">
                        <p dangerouslySetInnerHTML={{ __html: product?.long_description || "Không có chi tiết" }} />
                      </div>
                    </div>
                  </div>
                  ...
                  <div id="tab04" className={`tab-pane ${activeTab === "tab04" ? "active" : "fade"}`}>
                    <div className="row">
                      <div className="col-md-12">
                        {/* Form tư vấn */}
                        <div className="consult-form">
                          ...
                          >
                            {isLoadingConsult ? <i className="fa fa-spinner fa-spin" /> : "Gửi câu hỏi"}
                          </button>
                        </div>
                        ...
                        <div className="consult-history mt-4">
                          <div className="d-flex justify-content-between align-items-center">
                            ...
                            {consultHistory.length > 0 && (
                              <button className="btn btn-danger btn-sm" onClick={clearConsultHistory}>
                                Xóa lịch sử
                              </button>
                            )}
                          </div>
                          {consultHistory.length === 0 ? (
                            <p>Chưa có câu hỏi nào được đặt. Hãy gửi câu hỏi để nhận tư vấn!</p>
                          ) : (
                ...
                {relatedProducts.map((item) => (
                  <div key={item._id} className="product">
                      ...
                      <div>
                        <h4 className="product-price">
                          ...
                          {item.variants[0]?.salePrice !== item.variants[0]?.price && (
                            <del className="product-old-price">{formatPrice(item.variants[0]?.price ?? 0)}</del>
                          )}
                        </h4>
                    ...
                    <div className="add-to-cart">
                      <button
                        className="add-to-cart-btn"
                        onClick={() => addToCart(item._id, item.variants?.[0], 1)}
                      >
                        <i className="fa fa-shopping-cart" /> Thêm giỏ hàng
                      </button>
                    </div>
